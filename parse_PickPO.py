        ...
        with open("stock.txt") as s:
            ...
            for line in s:
                ...
                if "AND" in line:
                    ...
                    for section in split:
                        ...
                        if 'instr("123789", 20)' in section:
                            product_type = section[8] + "*"  + section[9] + "*" + section[10] + "*" + section[11] + "*" + section[12] + "*" + section[13]
                            print(product_type)
                        if ':instr("456", 20)' in section:
                            product_type = section[8] + "*" + section[9] + "*" + section[10]
                            print(product_type)
                        if "instr(" in section:
                            if "{ 3 {" in section:
                                if product_code_list != "null":
                ...
                if line.startswith(':mid(4,"6","1") = "') and " AND " not in line and " OR " not in line:
                    ...
                    #print(plus_or_minus)
                if line.startswith(':instr("456",20)') and " AND " not in line and " OR " not in line:
                    product_type = line[8] + "*" + line[9] + "*" + line[10]
                    ...
                    if "*" in product_code_list:
                        ...
                        if "*" not in quantity:
                            for product_code in product_code_list:
                                ...
                                "Sphere_2":sphere_2, "Product_Type": product_type})
                        if "*" in quantity:
...
def write_import(records):
    with open("import.csv", "w", newline="") as f:
        fieldnames = ["vendor", "S|P", "PO_Group", "Product_Code",
                      ...
                      "Diameter", "MF_Add_Power", "Cylinder", "Sphere_1", "Sphere_2", "Product_Type"]
